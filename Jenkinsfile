pipeline {
  ...
  stages {
    ...
    stage('Deploy with no traffic') {
      steps {
        sh("""
        gcloud alpha run deploy simple-http --namespace=default \
        --image=${IMAGE_URL} \
        --connectivity=external \
        --set-env-vars=TARGET=${BUILD_TAG} \
        --no-traffic \
    ...
    stage('Verification') {
      steps {
        sh("kubectl exec -it sleep-7b9758757b-7vm54 -- curl -f simple-http-${BUILD_TAG}-private/healthcheck")
      }
    }
    ...
    stage('50% Rollout tests') {
      steps {
        sh("kubectl exec -it sleep-7b9758757b-7vm54 -- curl -f http://simple-http.default.35.185.251.139.xip.io/healthcheck")
      }
    }
