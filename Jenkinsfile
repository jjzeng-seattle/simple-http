pipeline {                                                                                                                                                                                                  
  ...
  stages {                                                                                                                                                                                                  
    ...
    stage('Deploy with no traffic') {                                                                                                                                                                       
      steps {                                                                                                                                                                                               
        sh("""                                                                                                                                                                                              
        gcloud alpha run deploy simple-http --namespace=default --image=${IMAGE_URL} --connectivity=external --set-env-vars=BUILD=${BUILD_TAG} --no-traffic --revision-suffix=${BUILD_TAG}
        """)
      }
    }
    ...
    stage('50% Rollout tests') {
      steps {
        sh("curl -f http://simple-http.default.35.185.251.139.xip.io/healthcheck")
      }
    }
